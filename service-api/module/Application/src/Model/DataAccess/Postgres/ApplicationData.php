...
class ApplicationData extends AbstractBase implements ApplicationRepository\ApplicationRepositoryInterface
{
    ...
    public function getById(int $id, ?string $userId = null, ?string $sharedSpaceId = null): ?array
    {
        $criteria = ['id' => $id];
        if (!is_null($userId)) {
            $criteria[] = $this->resolveOwnerPredicate($userId, $sharedSpaceId);
        }
    ...
    {
        return $this->fetch([$this->ownerPredicate($userId)], $options);
    }
    ...
    public function getByIdsAndUserOrSharedSpace(array $lpaIds, string $userId, ?string $sharedSpaceId = null): Traversable
    {
        $criteria = [
            ...
            $this->resolveOwnerPredicate($userId, $sharedSpaceId),
        ];
    ...
    {
        return new Operator('sharedSpaceId', Operator::OPERATOR_EQUAL_TO, $sharedSpaceId);
    }

    /**
     * Resolve the correct ownership predicate for a call site that has both
     * a $userId and an optional $sharedSpaceId on hand - i.e. $userId's own
     * individually-owned LPAs if $sharedSpaceId is null, or all of the
     * shared space's LPAs if $sharedSpaceId is provided.
     *
     * @param string $userId
     * @param string|null $sharedSpaceId
     * @return PredicateInterface
     */
    private function resolveOwnerPredicate(string $userId, ?string $sharedSpaceId): PredicateInterface
    {
        if (!is_null($sharedSpaceId)) {
            return $this->sharedSpacePredicate($sharedSpaceId);
        }

        return $this->ownerPredicate($userId);
    }
